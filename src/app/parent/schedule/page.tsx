...
export default async function ParentSchedulePage() {
  ...
  const { data: guardian } = await supabase
    .from('guardians')
    .select('player:players(category)')
    .eq('profile_id', user.id)
    .single()

  const category = (guardian?.player as any)?.category ?? 'All'
  ...
  function SessionCard({ s, isNext }: { s: any; isNext?: boolean }) {
    return (
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-lg">{typeIcon(s.session_type)}</span>
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      <h1 className="page-title mb-1">Training Schedule</h1>
      ...
      {!upcoming?.length ? (
        ...
        <div className="space-y-3 mb-8">
          {upcoming.map((s, i) => <SessionCard key={s.id} s={s} isNext={i === 0}/>)}
        </div>
      )}
